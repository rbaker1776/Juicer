...
static constexpr std::string_view FENS[128]
{
...
static const std::vector<std::vector<uint64_t>> PERFT_RESULTS
{
...
int main()
{
	...
	for (size_t i = 0; i < 128; ++i)
	{
		juicer.seed(FENS[i]);
		int depth = 0;
		for (uint64_t res: PERFT_RESULTS[i])
		{
			++depth;
			if (juicer.perft<false>(depth) != res)
			{
				std::cout << "\x1b[31m" << FENS[i] << std::endl;
				std::cout << "   Depth: " << depth << std::endl;
				std::cout << "Expected: " << res << std::endl;
				std::cout << "   Found: " << juicer.perft<false>(depth) << std::endl;
				std::cout << "\x1b[0m";
				return 1;
			}
		}
	...
	std::cout << "\x1b[32m" << "All tests passed." << "\x1b[0m" << std::endl;
